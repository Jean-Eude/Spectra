...
uniform bool grayscaledLap;
uniform bool invertLap;
uniform bool tresholdLap;
uniform float tresholdScaleLap;
...
for(int i=0;i<25;i++)
{
	color+= vec3(texture(tex0,texCoord.st+offset[i]))*G[i];
}


if(grayscaledLap == true)
{
	float grey = dot(color.rgb, vec3(0.2627f, 0.6780f, 0.0593f));
	FragColor = vec4(vec3(grey), 1.0f);
}
else if(invertLap == true)
{
	FragColor = vec4(1.0f - color,1.0f);
}
else if(tresholdLap == true)
{
	vec3 col = color.rgb;
    float bright = 0.33333f * (col.r + col.g + col.b);
    float b = mix(0.0f, 1.0f, step(tresholdScaleLap, bright));
